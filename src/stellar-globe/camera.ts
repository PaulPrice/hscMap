...
export function equal(p1: CameraParams, p2: CameraParams) {
    if (p1.mode != p2.mode)
        return false
    if (vec3.dist(math.radec2xyz(p1.a, p1.d), math.radec2xyz(p2.a, p2.d)) / p2.fovy > 1.e-3)
        return false
    if (Math.abs(Math.log(p2.fovy / p1.fovy)) > 1.e-3)
        return false
    if (Math.abs(p1.roll - p2.roll) > 1.e-3 || Math.abs(p1.tilt - p2.tilt) > 1.e-3)
        return false
    return true
}
