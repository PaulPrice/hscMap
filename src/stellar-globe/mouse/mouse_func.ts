...
class InertialEventFilter extends EventFilter {
    ...
    mousedown = (ev: MouseEvent) => {
        ...
        this.v = { x: 0, y: 0 }
        this.last = this.mouseFunc.globe.elementCoord(ev)
        this.anchor = this.mouseFunc.globe.elementCoord(ev) // set also in mousedown
        this.mouseFunc.mousedown(this.r)
        let animation = new MotionAnimation(this.mouseFunc.globe, {
            callback: ({ dt }) => {
