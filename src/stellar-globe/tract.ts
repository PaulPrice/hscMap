...
export class Tract {
    ...
    tileIndices(invPvMatrix: mat4, windowHeight: number, scaleFactor = 1, callback: (level: number, i: number, j: number) => void) {
        let invPvmMatrix = mat4.mul(mat4Tmp, this.invMMatrix, invPvMatrix)
        let { onScreen, cps } = crossPoint2D(invPvmMatrix, [[-1, -1], [-1, 1], [1, -1], [1, 1]])
        if (onScreen) {
            const tmp1 = ((cps[0][0] - cps[1][0]) * this.naxis1)
            const tmp2 = ((cps[0][1] - cps[1][1]) * this.naxis2)
            let scale2 = (tmp1 * tmp1 + tmp2 * tmp2) / windowHeight / windowHeight
            let level = Math.floor(Math.log(scaleFactor * scaleFactor * scale2) / 2 / Math.LN2)
            if (level < 0) level = 0
            ...
            if (minT <= 1 && maxT >= 0 && minU <= 1 && maxU >= 0) {
                let tileScale = this.tileSize * (1 << level)
