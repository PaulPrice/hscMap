...
class Globe(object):
    ...
    def focusTable(self, header, data, postageStamp=False, size=64, fov=10, onHover=None):
        ...
        print('<table id="{}" class="focusTable" data-coord-col="{}" data-globe-id="{}" data-size="{}" data-fov="{}" data-hover="{}"><thead><tr>'.format(
            tableId, coordCol, self.id, size, fov, hoverId
        ), file=html)
        print('<th>No.</th>', file=html)
        if postageStamp:
            print('<th style="font-size: 150%;">&#x1F600;</th>', file=html)
        for th in header:
            print('<th>', th, '</th>', file=html)
        print('</tr></thead><tbody>', file=html)
        for i, row in enumerate(data):
            print('<tr data-index="{}">'.format(i), file=html)
            print('<td>{}</td>'.format(i), file=html)
            if postageStamp:
                print('<td></td>', file=html)
            for td in row:
                print('<td>', td, '</td>', file=html)
            print('</tr>', file=html)
        print('</tbody></table>', file=html)
